...
show_classic_menu() {
    ...
    echo -e "=============================================================================${NC}"
    echo "1. Install LAMP Stack + WordPress   ./run.sh lamp"
    echo "2. Backup WordPress Sites           ./run.sh backup"
    echo "3. Restore WordPress Sites          ./run.sh restore"
    echo "4. Transfer Backups to Cloud        ./run.sh transfer"
    echo "5. Configure MySQL Remote Access    ./run.sh mysql"
    echo "6. Check MySQL Remote Access        ./run.sh checkdb"
    echo "7. Show MySQL Databases             ./run.sh showdb"
    echo "8. List MySQL Users                 ./run.sh users"
    echo "9. Get Database Size                ./run.sh dbsize"
    echo "10. Install phpMyAdmin              ./run.sh phpmyadmin"
    echo "11. Enable MySQL Log Purging        ./run.sh purge"
    echo "12. Adjust PHP Settings             ./run.sh php"
    echo "13. View PHP Information            ./run.sh phpinfo"
    echo "14. System Status Check             ./run.sh status"
    echo "15. Disk Space Monitor              ./run.sh disk"
    echo "16. Toggle Root SSH Access          ./run.sh ssh"
    echo "17. Install System Utilities        ./run.sh utils"
    echo "18. Install Rclone Package          ./run.sh rclone"
    echo "19. Configure Rclone Remote         ./run.sh config"
    echo "20. Show Rclone Remotes             ./run.sh remotes"
    echo "21. Check Rclone Status             ./run.sh rclonestatus"
    echo "22. Manage Cloud Remote             ./run.sh manage"
    echo "23. Setup Backup Automation         ./run.sh cron"
    echo "24. Uninstall Rclone                ./run.sh uninstall"
    echo "25. Configure Redis Cache           ./run.sh redis"
    echo "26. Install Apache + SSL Only       ./run.sh ssl"
    echo "27. Fix Apache Configurations       ./run.sh fixapache"
    echo "28. Remove Websites & Databases     ./run.sh remove"
    echo "29. Remove Orphaned Databases       ./run.sh cleanup"
    echo "30. Troubleshooting Tools           ./run.sh troubleshoot"
