...
show_menu() {
    ...
    echo "7. Show MySQL Databases             ./main.sh showdb       # Display all databases in MySQL server"
    echo "8. List MySQL Users                 ./main.sh users        # Show all MySQL user accounts and permissions"
    echo "9. Get Database Size                ./main.sh dbsize       # Check storage usage of databases"
    echo "10. Install phpMyAdmin              ./main.sh phpmyadmin   # Set up web-based MySQL administration"
    echo "11. Enable MySQL Log Purging        ./main.sh purge        # Configure automatic binary log cleanup"
    echo "12. Adjust PHP Settings             ./main.sh php          # Optimize PHP configuration for web apps"
    echo "13. View PHP Information            ./main.sh phpinfo      # Display PHP version and configuration"
    echo "14. System Status Check             ./main.sh status       # View system resources and service status"
    echo "15. Disk Space Monitor              ./main.sh disk         # Monitor storage usage and cleanup"
    echo "16. Toggle Root SSH Access          ./main.sh ssh          # Enable or disable SSH root login"
    echo "17. Install System Utilities        ./main.sh utils        # Install common system tools"
    echo "18. Install Rclone Package          ./main.sh rclone       # Download and install rclone with dependencies"
    echo "19. Configure Rclone Remote         ./main.sh config       # Set up new cloud storage authentication"
    echo "20. Show Rclone Remotes             ./main.sh remotes      # Display configured remotes and accessibility"
    echo "21. Check Rclone Status             ./main.sh rclonestatus # View rclone setup and configuration status"
    echo "22. Manage Cloud Remote             ./main.sh manage       # Interactive cloud storage management"
    echo "23. Setup Backup Automation         ./main.sh cron         # Configure automatic backup scheduling"
    echo "24. Uninstall Rclone                ./main.sh uninstall    # Remove rclone and all configurations"
    echo "25. Configure Redis Cache           ./main.sh redis        # Set up Redis caching for performance"
    echo "26. Install Apache + SSL Only       ./main.sh ssl          # Set up web server with SSL certificates"
    echo "27. Fix Apache Configurations       ./main.sh fixapache    # Repair broken Apache virtual host configs"
    echo "28. Remove Websites & Databases     ./main.sh remove       # Clean removal of websites and data"
    echo "29. Remove Orphaned Databases       ./main.sh cleanup      # Clean up databases without websites"
    echo "30. Troubleshooting Tools           ./main.sh troubleshoot # Diagnose and fix common issues"
    echo "0. Exit"
    echo -e "${CYAN}=============================================================================${NC}"
}
