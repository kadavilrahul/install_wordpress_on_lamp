...
show_header() {
    ...
    echo "                     Diagnostics and Problem Solving"
    echo "============================================================================="
    echo -e "${NC}"
...
handle_cli_command() {
    ...
    case $command in
        "troubleshoot"|"menu") execute_script "$SCRIPT_DIR/troubleshooting_menu.sh" "Troubleshooting Menu" ;;
        "dbfix"|"guide") 
            if [ -f "$SCRIPT_DIR/DATABASE_FIX.md" ]; then
                less "$SCRIPT_DIR/DATABASE_FIX.md"
            else
...
main() {
    ...
    while true; do
        ...
        case $choice in
            1) execute_script "$SCRIPT_DIR/troubleshooting_menu.sh" "Troubleshooting Menu" ;;
            2) 
                if [ -f "$SCRIPT_DIR/DATABASE_FIX.md" ]; then
                    less "$SCRIPT_DIR/DATABASE_FIX.md"
                else
