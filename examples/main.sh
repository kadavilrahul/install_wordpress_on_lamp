...
restore_wordpress() {
    ...
    readarray -t backup_files < <(find "$BACKUP_DIR" -name "*.tar.gz" -type f | sort)
    ...
    if [ -d "$WWW_PATH/$TARGET_SITE" ]; then
        confirm "Overwrite existing directory?" || { info "Cancelled"; read -p "Press Enter..."; return; }
        rm -rf "$WWW_PATH/$TARGET_SITE"
    fi
    
    info "Restoring $(basename "$selected_backup") to $TARGET_SITE..."
    tar -xzf "$selected_backup" -C "$WWW_PATH" || error "Failed to extract backup"
    
    target_dir="$WWW_PATH/$TARGET_SITE"
    rm -f "$target_dir/wp-content/object-cache.php" "$target_dir/wp-content/advanced-cache.php"
    
    # Find and import database
    for db_dump in "$target_dir"/*_db.sql "$target_dir"/*.sql; do
        if [ -f "$db_dump" ]; then
            [ -f "$target_dir/wp-config.php" ] && wp db import "$db_dump" --path="$target_dir" --allow-root 2>/dev/null && rm -f "$db_dump" && break
        fi
    done
    
    chown -R www-data:www-data "$target_dir" && chmod 755 "$target_dir"
