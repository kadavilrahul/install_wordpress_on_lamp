...
LOG_FILE="/var/log/wordpress_installation_$(date +%Y%m%d_%H%M%S).log"
...
show_header() {
    ...
    echo "                   Install WordPress on existing LAMP stack"
    echo "============================================================================="
    echo -e "${NC}"
...
check_lamp_installed() {
    info "Checking if LAMP stack is installed..."
    
    local lamp_ok=true
    
    # Check Apache
    if ! command -v apache2 &>/dev/null || ! systemctl is-active --quiet apache2; then
        error "Apache is not installed or not running. Please install LAMP stack first."
    fi
    
    # Check MySQL
    if ! command -v mysql &>/dev/null || ! systemctl is-active --quiet mysql; then
        error "MySQL is not installed or not running. Please install LAMP stack first."
    fi
    
    # Check PHP
    if ! command -v php &>/dev/null; then
        error "PHP is not installed. Please install LAMP stack first."
    fi
    
    success "LAMP stack is installed and running"
}
...
get_inputs() {
    local type="$1"
    echo -e "${YELLOW}WordPress Installation Configuration:${NC}"
    ...
    confirm "Proceed with WordPress installation?" || return 1
}
...
ORIGINALLY_ENABLED_SITES=()

# Function to save currently enabled sites
save_enabled_sites() {
    info "Saving list of currently enabled sites..."
    ORIGINALLY_ENABLED_SITES=()
    ...
    for site_conf in /etc/apache2/sites-enabled/*.conf; do
        if [ -L "$site_conf" ] && [ -e "$site_conf" ]; then
            ...
            info "Saved enabled site: $site_name"
        fi
    done
    
    success "Saved ${#ORIGINALLY_ENABLED_SITES[@]} enabled sites"
}

# Function to restore originally enabled sites
restore_enabled_sites() {
    info "Restoring originally enabled sites..."
        ...
        if [ -f "/etc/apache2/sites-available/$site" ]; then
            ...
            if [ ! -L "/etc/apache2/sites-enabled/$site" ]; then
                if a2ensite "$site" 2>/dev/null; then
                    info "Re-enabled: $site"
                    ((restored_count++))
                else
                    warn "Failed to re-enable: $site"
                    ((failed_count++))
                fi
            else
            ...
            warn "Site config not found: $site (may have been removed)"
        fi
    done
    
    if [ $restored_count -gt 0 ] || [ $failed_count -gt 0 ]; then
        systemctl reload apache2 2>/dev/null
    fi
    
    success "Site restoration complete: $restored_count restored, $failed_count failed"
}
...
EOF
    ...
    # Restore all originally enabled sites
    restore_enabled_sites
}
